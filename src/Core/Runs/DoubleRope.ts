...
export class DoubleRope implements IRun {
  run: Run;
  ropeRun: Run;
  widthMm: number;
  constructor(
  ...
  getStitches(pixelsPerMm: number): Stitch[] {
    ...
    for (let i = 0; i < ropeStitches.length; i++) {
      ...
      if (i % 2 === 0) stitches.push(new Stitch(curr.add(normal), StitchType.NORMAL));
      else stitches.push(new Stitch(curr.subtract(normal), StitchType.NORMAL));
    }
